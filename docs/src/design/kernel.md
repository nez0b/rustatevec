...
Applying a 1-qubit gate \\(G = \begin{psmallmatrix} g_{00} & g_{01} \\\\ g_{10} & g_{11}
\end{psmallmatrix}\\) to qubit \\(q\\) couples exactly the amplitude pairs whose basis indices
...
This is the same arithmetic as `expand_int` in the reference notebook
(`docs/reference/state_vector.jl`), QuEST's `insertZeroBit`, Aer's `index0`, and qsim's
bit-deposit.
